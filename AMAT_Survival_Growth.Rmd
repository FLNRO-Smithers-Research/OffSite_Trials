...
file.name <- "./data/AMAT_MASTERDATA_ver_14_seedlots.csv"
seedlots <- fread(file.name) %>% dplyr::select(SLnum, SLtype)


file.name <- "./data/AMAT_MASTERDATA_ver_14.csv"

data <- fread(file.name) %>% left_join(seedlots) %>% dplyr::select(Site, Sp, SLnum, SLtype, Rep, HT10, Comments10) %>% mutate(HT10 = replace(HT10, HT10 == "9999", 0)) %>% mutate(HT10 = replace(HT10, HT10 == ".", 0)) %>% mutate(HT10 = as.numeric(HT10)) %>% mutate(spp = ifelse(SLtype == "LC", paste0(Sp, SLtype), Sp)) %>% 
...
HT_viol_10 <- ggplot(data1, aes(x= spp, y=HT10, colour = Sp))+
